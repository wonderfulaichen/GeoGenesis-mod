...
## 14. 2026-09-19 追加：中断修复的【实机端到端验证】

用户实机跑一遍（seed 9139912035078620160，3847 chunk，252.3 s 会话）。

### 14.1 ★ 100 秒离群彻底消失

| 指标 | 修复前（1197 块） | **修复后（3847 块）** |
|---|---|---|
| `侵蚀tile提取` 总耗时 | **112331 ms** | **14787 ms** |
| 占比 | **81.3%** | **5.6%** |
| 均 | 93.8 ms | **3.8 ms** |
| **max** | **103426 ms** | **732 ms** |

**块数多了 3.2 倍，提取总耗时反而降到 1/7.6**（总耗时 −87%）。

| 世界创建 | 原始 | 修预览泄漏后 | 本轮（+中断修复） |
|---|---|---|---|
| 耗时 | 37963 ms | 24876 / 25119 ms | **24774 ms** |
...
```
地形采样      3847    13129.4 ms   均 3.413   P50 3.194   max 16.292    4.9%
侵蚀tile提取  3847    14787.9 ms   均 3.844   P50 0.132   max 731.961   5.6%
水文雕刻      3847    56346.3 ms   均 14.647  P50 10.816  max 2474.485  21.2%
  ├ 水文-计算 3847    51538.0 ms   均 13.397  P50 10.682  max 2474.470  19.4%   ← 91.5%
  ├ 水文-河距 3847       21.0 ms   均 0.005                                0.0%
  └ 湖岸精修   956     4205.3 ms   均 4.399   P50 0.011   P95 25.186     1.6%
方块铺设      3847    32328.5 ms   均 8.404                             12.1%
侵蚀tile生成   133    19308.3 ms   均 145.2   max 251.6                  7.3%
tile等待(阻塞) 121    17296.5 ms   均 142.9   max 251.6  ≈ 生成耗时 ⇒ 无阻塞
停顿(未占CPU)  697    13786.0 ms   均 19.8   P50 6.0     max 1106.9      5.2%
块均总耗时 = 30.02 ms（修复前 56.08 ms）
```

### 14.3 结论与下一步

- ✅ **离线复现 + 离线修复 + 反向验证 → 实机确认有效**（`extract` max 103s → 0.7s）。
  这条链路证明：能用离线探针复现时，**不必反复让用户开游戏**。
- ⚠️ **新瓶颈已明确**：`水文-计算`（`hydrologyExperiment.calculate`）**13.4 ms/块、
  占单块时间的 45%**，是现在最大的常态开销。
- **已定位可省项**：`calculate()` 第一行 `sampleOriginal()` **重新采样整块 256 个 Cell**
  （只为拿无侵蚀基线）。而 `generateChunk` 采样循环【在侵蚀之前】的产出与之**逐位相同**
  （同为 `generator.sample(block/hs)`；`toWu` 在 hs==1 时返回 block，与 /1.0 同值）
  ⇒ 可在侵蚀前【深拷贝快照】传入，省掉这次重采样。
  量级：地形采样 3.413 ms/块 ⇒ 约省 **3.4 ms/块（单块时间 −11%）**。
- ⚠️ 埋点口径问题（记下）：`提取内-*` 三个子项**也被结构放置 / 群系快速路径调用**
  （`applyCachedTileDelta` / `peekErosionDeltaE`），不只走 `extractFromTile`
  ⇒ 子项之和会【大于】`侵蚀tile提取`。读数时注意。
