...
public record MidSplineConfig(
    ...
    public static MidSplineConfig defaults() {
        return new MidSplineConfig(
            // 节点 0（深海 c=-0.80）：只有 BASIN 有权重（模拟深海盆地）
            0.0,  0.0, 0.0,  // PLAIN: 无
            0.25, 0.0, 0.0,  // HILLS: 无
            0.5,  0.0, 0.0,  // MOUNTAINS: 无
            0.75, 0.0, 0.0,  // PLATEAU: 无
            1.0,  1.0, 0.0,  // BASIN: 权重=1.0

            // 节点 1（大陆架 c=-0.50）：只有 PLAIN 有权重（模拟平坦海底）
            ...
            // 节点 2（浅海 c=-0.16）：只有 PLAIN 有权重（模拟浅海平原）
            0.0,  1.0, 0.0,  // PLAIN: 权重=1.0
            0.25, 0.0, 0.0,  // HILLS: 无
            0.5,  0.0, 0.0,  // MOUNTAINS: 无
            0.75, 0.0, 0.0,  // PLATEAU: 无
            ...
            // 节点 3（海岸 c=-0.04）：PLAIN + HILLS（模拟海岸丘陵，丘陵向海岸延伸）
            0.0,  0.6, 0.0,  // PLAIN: 权重=0.6
            0.25, 0.4, 0.0,  // HILLS: 权重=0.4
            0.5,  0.0, 0.0,  // MOUNTAINS: 无
            0.75, 0.0, 0.0,  // PLATEAU: 无
            1.0,  0.0, 0.0,  // BASIN: 无

            // 节点 4（近岸陆地 c=0.20）：PLAIN→HILLS→MOUNTAINS 过渡序列（丘陵缓冲，近岸已有低山）
            0.0,  0.30, 0.0,  // PLAIN: 权重=0.30
            0.25, 0.35, 0.0,  // HILLS: 权重=0.35
            0.5,  0.25, 0.0,  // MOUNTAINS: 权重=0.25（0.15 会让低 c 区山地消失——探针 0 样本）
            0.75, 0.05, 0.0,  // PLATEAU: 权重=0.05
            1.0,  0.05, 0.0,  // BASIN: 权重=0.05

            // 节点 5（内陆 c=0.50）：HILLS + MOUNTAINS + PLATEAU（PLAIN 收窄到海岸，序列拉开）
            0.0,  0.10, 0.0,  // PLAIN: 权重=0.10
            0.25, 0.30, 0.0,  // HILLS: 权重=0.30
            0.5,  0.30, 0.0,  // MOUNTAINS: 权重=0.30
            0.75, 0.15, 0.0,  // PLATEAU: 权重=0.15
            1.0,  0.15, 0.0,  // BASIN: 权重=0.15

            // 节点 6（深远内陆 c=0.80）：MOUNTAINS + PLATEAU 主导（模拟深远内陆山脉/高原）
            0.0,  0.05, 0.0,  // PLAIN: 权重=0.05
            0.25, 0.15, 0.0,  // HILLS: 权重=0.15
            0.5,  0.45, 0.0,  // MOUNTAINS: 权重=0.45
            0.75, 0.25, 0.0,  // PLATEAU: 权重=0.25
            1.0,  0.10, 0.0   // BASIN: 权重=0.10
        );
    }
