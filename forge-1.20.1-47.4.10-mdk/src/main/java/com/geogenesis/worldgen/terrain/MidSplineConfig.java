...
public record MidSplineConfig(
    ...
    public static MidSplineConfig defaults() {
        return new MidSplineConfig(
            ...
            // 节点 3（海岸 c=-0.04）：PLAIN + HILLS（模拟海岸丘陵）
            0.0,  0.7, 0.0,  // PLAIN: 权重=0.7
            0.25, 0.3, 0.0,  // HILLS: 权重=0.3
            0.5,  0.0, 0.0,  // MOUNTAINS: 无
            0.75, 0.0, 0.0,  // PLATEAU: 无
            1.0,  0.0, 0.0,  // BASIN: 无

            // 节点 4（近岸陆地 c=0.20）：PLAIN + HILLS + MOUNTAINS（模拟近岸多样化地形）
            // （2026-08-04：高原权重 0.1→0.15、平原 0.4→0.35，使高原在陆地稍多见，节点权重和仍=1）
            0.0,  0.35, 0.0,  // PLAIN: 权重=0.35
            0.25, 0.3, 0.0,  // HILLS: 权重=0.3
            0.5,  0.2, 0.0,  // MOUNTAINS: 权重=0.2
            0.75, 0.15, 0.0,  // PLATEAU: 权重=0.15
            1.0,  0.0, 0.0,  // BASIN: 无

            // 节点 5（内陆 c=0.50）：所有类型都有权重（模拟内陆多样化地形）
            // （2026-08-04：高原权重 0.1→0.18、平原 0.3→0.22，节点权重和仍=1）
            0.0,  0.22, 0.0,  // PLAIN: 权重=0.22
            0.25, 0.3, 0.0,  // HILLS: 权重=0.3
            0.5,  0.2, 0.0,  // MOUNTAINS: 权重=0.2
            0.75, 0.18, 0.0,  // PLATEAU: 权重=0.18
            1.0,  0.1, 0.0,  // BASIN: 权重=0.1

            // 节点 6（深远内陆 c=0.80）：MOUNTAINS + PLATEAU 主导（模拟深远内陆山脉/高原）
            // （2026-08-04：高原权重 0.2→0.3、山脉 0.4→0.3，二者同为隆起高地配对，节点权重和仍=1）
            0.0,  0.1, 0.0,  // PLAIN: 权重=0.1
            0.25, 0.2, 0.0,  // HILLS: 权重=0.2
            0.5,  0.3, 0.0,  // MOUNTAINS: 权重=0.3
            0.75, 0.3, 0.0,  // PLATEAU: 权重=0.3
            1.0,  0.1, 0.0   // BASIN: 权重=0.1
        );
    }
