...
public record SplineConfig(
    ...
    public static SplineConfig defaults() {
        return new SplineConfig(
            ...
            // HILLS inner spline (12): lo=[0.06,0.06], hi=[0.368,0.368]
            // （2026-08-05 用户目标：丘陵最高默认 ~150 格 → hi=e≈0.368；lo 0.06→77 格，
            //  内部落差 ~73 格，山丘明显。注意实际地形受调制系数影响，峰值通常略低于 hi）
            0.0, 0.06,  0.0, 1.0, 0.06,  0.0,
            0.0, 0.368, 0.0, 1.0, 0.368, 0.0,
            // MOUNTAINS inner spline (12): lo=[0.45,0.45], hi=[0.95,0.95]
            0.0, 0.45,  0.0, 1.0, 0.45,  0.0,
            0.0, 0.95,  0.0, 1.0, 0.95,  0.0,
            // PLATEAU inner spline (12): lo=[0.41,0.41], hi=[0.706,0.706]
            // （2026-08-05 用户目标：高原 160/230 格 → lo≈0.41、hi≈0.706；台面落差 ~70 格）
            0.0, 0.41,  0.0, 1.0, 0.41,  0.0,
            0.0, 0.706, 0.0, 1.0, 0.706, 0.0,
            // BASIN inner spline (12): lo=[-0.08,-0.08], hi=[0.05,0.05]
            // ★ 2026-09-14 从 hi=0.02 上调到 0.05（用户反馈"靠海的地形类型中出现盆地"）：
            //   原 hi=0.02 e 仅海平面上 3.8 块，lo=-0.08 e 在海面下 15 块
            //   ⇒ 盆地类型的高度区间【骑在海平面线上】⇒ 天然贴着海岸线出现。
            //   （对照 PLAIN 0.005~0.03 e 同样贴海面 —— 即"海岸低地被判为盆地"。）
            //   抬到 0.05 后盆顶达海平面上 9.6 块，与 PLAIN 区间错开 ⇒ 不再与海岸混同。
            //   lo 保持 -0.08：保留内陆沉降洼地/裂谷成湖的能力（地质语义不变）。
