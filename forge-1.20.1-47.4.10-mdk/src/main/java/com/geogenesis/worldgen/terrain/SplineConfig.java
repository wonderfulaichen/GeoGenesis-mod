...
public record SplineConfig(
    ...
    public static SplineConfig defaults() {
        return new SplineConfig(
            ...
            // HILLS inner spline (12): lo=[0.02,0.02], hi=[0.12,0.12]
            // （2026-08-01 大幅下调：丘陵实际高度 Y∈[85,130]（e≈[0.10,0.28]，1 格≈10~20 米语义）。
            //  注意 blend 混合稀释：HILLS 核心区（权重 0.7）混入 30% 其他类型 → 样条必须远低于目标值
            //  才能让混合后落在目标区间；旧 0.09/0.24 混合后 mean 141 仍偏高（+11 格））
