...
public record SplineConfig(
    ...
    public static SplineConfig defaults() {
        return new SplineConfig(
            ...
            // 【2026-08-01 高度带连续衔接】类型差异化靠起伏幅度（PLAIN 平/HILLS 起伏/PLATEAU 台地/
            // MOUNTAINS 高耸），高度带首尾相接（hi(前)=lo(后)）→ 高度场连续自然变化，无平台-斜坡-平台阶梯感：
            //   PLAIN [0.015,0.06] → HILLS [0.06,0.35] → PLATEAU [0.35,0.50] → MOUNTAINS [0.50,0.95]
            // HILLS inner spline: lo=[0.06,0.06]=PLAIN hi、hi=[0.35,0.35]=PLATEAU lo
            0.0, 0.06,  0.0, 1.0, 0.06,  0.0,
            0.0, 0.35,  0.0, 1.0, 0.35,  0.0,
            // MOUNTAINS inner spline: lo=[0.50,0.50]=PLATEAU hi、hi=[0.95,0.95]
            0.0, 0.50,  0.0, 1.0, 0.50,  0.0,
            0.0, 0.95,  0.0, 1.0, 0.95,  0.0,
            // PLATEAU inner spline: lo=[0.35,0.35]=HILLS hi、hi=[0.50,0.50]=MOUNTAINS lo
            // （台地带 [0.35,0.50] 位于丘陵之上、山脉之下——地理上高原常为山脉边缘台地；平顶形态由类型噪声保证）
            0.0, 0.35,  0.0, 1.0, 0.35,  0.0,
            0.0, 0.50,  0.0, 1.0, 0.50,  0.0,
