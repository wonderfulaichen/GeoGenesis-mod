...
public record SplineConfig(
    ...
    public static SplineConfig defaults() {
        return new SplineConfig(
            ...
            // HILLS inner spline (12): lo=[0.12,0.12], hi=[0.34,0.34]
            // （2026-07-31 带宽 0.15→0.22：实测 modulated 在类型区波动仅 ~0.4×range，
            //  0.15 带宽 → 内部落差仅 15 块（像平原）；0.22 → ~32 块（丘陵起伏合理）。
            //  低端 0.12 略高于陆地掩码保护带(h<0.10)，不被脊谷切穿）
            0.0, 0.12,  0.0, 1.0, 0.12,  0.0,
            0.0, 0.34,  0.0, 1.0, 0.34,  0.0,
            // MOUNTAINS inner spline (12): lo=[0.55,0.55], hi=[0.95,0.95]（谷底抬升，与 PLATEAU 分离）
            0.0, 0.55,  0.0, 1.0, 0.55,  0.0,
            0.0, 0.95,  0.0, 1.0, 0.95,  0.0,
            // PLATEAU inner spline (12): lo=[0.40,0.40], hi=[0.50,0.50]
            // （2026-07-31 从 [0.60,0.75] 下移：原带完全嵌套 MOUNTAINS [0.45,0.95] → 高原=山脉围边。
            //  新带 [0.40,0.50] 独立于 HILLS(≤0.40)/MOUNTAINS(≥0.55)，且 lo=LAND_REF → 脊谷不抬不切，平顶稳定）
            0.0, 0.40,  0.0, 1.0, 0.40,  0.0,
