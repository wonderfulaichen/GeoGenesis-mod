 ...
 * </pre>
 *
 * <p><b>★ 勘误（2026-09-16 二次核查）：本条初版断言"那个 [G] 探针根本不在仓库里"是错的。</b>
 * 它<b>存在</b> —— 就在 {@code TerrainStripePngProbe} 的 <b>[G] 段</b>（AGENTS 原文其实写明了
 * "新增 {@code runTerrainStripePngProbe} 的 [G] 段"）。当初误判的原因很简单：
 * 按 <b>560 / 水平直段</b> 字面搜索没命中 —— 该段实际打印的是
 * "最长<b>水平走向</b>直段=800 wu"，<b>用词不同</b> ⇒ <b>"搜不到"被当成了"不存在"</b>。
 * （与 README/文档那轮"关键词没命中就判定过时"是同一类错误。）</p>
 *
 * <p><b>那为何仍需要本探针？</b>三点，均经实测确认：</p>
 * <ol>
 *   <li><b>[G] 是单窗口单种子</b>：实测读数随种子在 0.28~3.20 漂移 ⇒ <b>不可用于决策</b>（见下）；</li>
 *   <li><b>[G] 的判据文字自相矛盾</b>：它打印"最长竖直走向直段 = <b>952 wu（0.32×）</b>"，
 *       却在同段下结论"<b>最值 ≪1.00 … ⇒ 类型边界无轴对齐倾向</b>" ——
 *       <b>0.32 不是"≪1.00"</b>，该结论无法由它自己的数字支撑；</li>
 *   <li><b>[G] 只测轴向</b>（水平/竖直走向），<b>对 45° 对角直段完全盲</b>
 *       ⇒ 无法回答"边界是不是一条条直线"（见升级二）。</li>
 * </ol>
 *
 * <p><b>★ 交叉验证（两套独立实现互证，指标本身可信）</b>：本探针（多相位中位）得轴向直段
 * <b>0.307~0.315×</b>；[G]（单窗口）得 <b>0.27 / 0.32×</b> ——
 * <b>两条独立代码路径给出同一量级</b>。</p>
 *
 ...
 *
 * <h2>★ Step 0 已闭环：缺陷【可见性】已确认（2026-09-16）</h2>
 * <p>预研 §6 Step 0 的出口问题是"<b>缺陷是否肉眼可见</b>"—— 它决定"值不值得冒风险修"。
 * 已用 {@code runTerrainStripePngProbe} 的渲染图回答：<b>可见</b>。</p>
 * <p>{@code build/stripe/type_dominant.png}（seed=5436529513624899584、size=1000、step=4）
 * 上可直接看到<b>贯穿性的竖直直线边界</b>与长水平直边，与 [G] 报出的 800/952 wu 一致。</p>
 * <p>⇒ <b>结论：该缺陷值得修</b>；但方案 B 的关键前提（"归属逐点不变"）尚未证明，
 * 详见 {@code docs/plans/轴向对齐与域扭曲-预研-2026-09-16.md}。</p>
 *
 * <h2>判据是【锚定】而非【达标】</h2>
