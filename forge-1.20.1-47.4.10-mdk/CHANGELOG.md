...
- **★ `runTypeAxisProbe` 升级为【多种子 × 多相位】聚合 + 修掉一个判据盲区（2026-09-16）**：
  - **起因**：类型场轴向对齐缺陷（P1）此前**所有结论都建立在"单种子 + 单窗口"读数上**，
    而该读数实测**随种子在 0.28~3.20 之间漂移**（跨种子 sd 中位 **0.53**，n=64）
    ⇒ 单点读数**根本不能用于决策**（重犯「单次测量下结论」）。
  - **升级**：扫 `种子 × 相位` 笛卡尔积（相位 `0/96/192/288`，对 `CELL_SPACING=400` 取模，
    且为 `step` 整数倍 ⇒ 只动 Voronoi 相位、不动采样格点相位），输出逐样本表 +
    聚合中位/p90 + ★**方差分解**。
  - **发现1（★ 纠正一条旧判断）**：跨种子 sd **0.396~0.531** ≫ 跨相位 sd **0.080~0.087**
    （差 5~6 倍）⇒ **AGENTS 里"另一窗口方向比 2.35 属窗口地理取样"应改述为"属种子差异"**
    （相位贡献 < 1/5）。
  - **发现2（★ 判据盲区，最重要）**：`方向比`（轴向/对角）对**方形格点的 45° 等长直段是瞎的**
    —— 方形格点下 0°/45°/90° **三个方向都又长又直** ⇒ 两边都长、比值 ≈1
    ⇒ **判据通过，而边界照样是一条条长直线**。
    实测：方向比中位 **1.11**（"达标" ≤1.5）而最长轴向直段 **中位 0.307×窗口 ≈ 920 块**
    （历史记录 560 块）⇒ **新增判据3「绝对直段长」补盲区**（相对量 + 绝对量互补）。
  - **发现3（★ 判据设计的教训：两面都栽过）**：拿单点数值去框一个**重尾分布**，
    **太松**（初版单样本 ≤2.6 ⇒ 换种子到 3.20 即**漏报**）与
    **太严**（第二版 n 样本 max ≤2.6 ⇒ 8 种子集 3.20 即**误报**，而**代码一字未改**）
    **是同一个病** ⇒ 判据改用**分布统计量**：
    判据1 中位 ≤1.5 · 判据2 **p90** ≤2.6 · 判据3 直段中位 ≤0.40×窗口；
    **`max` 只报告、不判定**。
  - **锚定基线（可复现）**：n=64（16 种子×4 相位）中位 **1.11** / p90 **1.87** / 直段中位 **0.307×**；
    n=20（默认 5 种子）为 **1.16 / 1.60 / 0.315×** ⇒ 三条判据余量均 **27~39%**。
  - **实测指纹**：`seed=777 / 42` 在 4 个相位下方向比**精确不变**（1.00 / 1.16）
    ⇒ 最长直段长度由**格点几何**决定、与窗口如何切割无关
    ⇒ **"相位不变"本身即"这是格点伪影"的判据**（值得后人记住）。
  - ⚠ **纯诊断源集改动**（`src/diagnostics`）⇒ **不影响任何产出 ⇒ 不 bump
    `PreviewDisplay.CACHE_SCHEMA_VERSION`**、无需实机复验。
  - **★ 勘误与闭环（同日续查）**：本条初版断言"那个 [G] 探针根本不在仓库里"—— **错了**。
    它**存在**：`TerrainStripePngProbe` 的 **[G] 段**（AGENTS 原文即写明"新增
    `runTerrainStripePngProbe` 的 [G] 段"）。误判原因是按字面搜 `560 / 水平直段`
    未命中 —— 它实际打印的是"最长**水平走向**直段 = 800 wu"，**用词不同**
    ⇒ **"搜不到"被当成了"不存在"**（与 README 那轮"关键词没命中就判过时"同类错误）。
    - **★ 交叉验证（指标本身可信）**：本探针（多相位中位）轴向直段 **0.307~0.315×**
      vs [G]（单窗口）**0.27 / 0.32×** ⇒ **两条独立代码路径给出同一量级**。
    - **★ [G] 判据自相矛盾**：它打印"最长竖直直段 = **952 wu（0.32×）**"，
      同段结论却是"**最值 ≪1.00 … ⇒ 类型边界无轴对齐倾向**" ——
      **0.32 不是"≪1.00"**，该结论无法由它自己的数字支撑；
      且 [G] **只测轴向**（水平/竖直走向），**对 45° 对角直段完全盲**。
    - **★ Step 0 闭环 —— 缺陷"可见性"已确认（预研 §6 的出口问题）**：
      `build/stripe/type_dominant.png`（seed=5436529513624899584、size=1000、step=4）上
      **肉眼可见贯穿性的竖直直线边界**与长水平直边，与 [G] 报出的 800 / 952 wu 一致
      ⇒ **该缺陷值得修**。
    - **仍未做**：方案 B 的关键前提（"归属逐点不变"）**尚未证明**；实机目视仍建议补。

### 修复 / Fixed

- **★★★ 类型场【轴向对齐】缺陷已修 —— 启用域扭曲（`WARP_AMP` 0→40）（2026-09-16）**：
  - **缺陷**：`TerrainCharacterField` 的格点在**规则方阵**上（`CELL_SPACING=400`），
    其 Voronoi 边界 = 相邻格点的**中垂线** ⇒ **必然轴对齐（0°/90° 或 45°）**。
    实测（16 种子×4 相位）：最长轴向直段 **中位 944 块（0.315×采样窗）**、
    **最大 1920 块（0.640×）**；渲染图 `type_dominant.png` 上
    **肉眼可见一条贯穿全图的竖直直线边界**。这是"规则网格范式固有属性"，
    非调参可消除（见 `docs/plans/轴向对齐与域扭曲-预研-2026-09-16.md`）。
  - **★ 推翻一条 2026-08-03 的旧结论（warp 被误诊）**：
    旧注释把"1 格宽断裂伪影"归因于 `WARP_AMP=80`。但**同一次改动还修了症状完全相同的
    另一处** —— `SEARCH_RADIUS` 1→3（3×3 → 7×7 窗口），其记录写明
    "typeWeights 在 1 格内突变 ~0.022 → argmax 翻转 → 1 格断裂线（**实测 @X=400/800**）"。
    **新增 `runWarpFractureProbe` 实证**（1 块步长、±256 窗口、amp=0/40/80/130/200 对照）：
    | amp | maxΔw（1 块） | 尖峰比 max/p99.9 | #(Δw > 0.022 参考线) |
    |---|---|---|---|
    | 0 | 0.00440 | 1.0 | 0 |
    | **80** | **0.00738** | **1.0** | **0** |
    | 200 | 0.01391 | 1.0 | 0 |
    ⇒ **amp=80 的最大跳变比历史"断裂"量级低 3 倍、超限格数 0、尖峰比恒 1.0
    （无孤立跳变）** ⇒ **当前代码下 warp 不产生断裂**。机理：`SEARCH_RADIUS=3` 下
    进出窗口的格点权重仅 **3.7e-6**（影响 ~2.6e-5），**这正是该参数的设计目的**
    ⇒ **断裂源已被窗口修复消除，warp 背了锅**。
    旁证：`TectonicField` 用同一机制（amp=130/波长 400）且实测已消除长直线、无断裂。
  - **决策：取 `amp = 40`（而非历史值 80）** —— 实测对照：
    | amp | 最长轴向直段中位 | `border.maxSurfaceDelta` | LandE 最坏跳变 |
    |---|---|---|---|
    | 0（旧） | 944 块（0.315×） | 1.845 | 10 次 / 0.02251e @(723,−755) |
    | **40（采用）** | **272 块（0.091×）** | **0.764** | 11 次 / 0.02257e **@(723,−755) 同位置** |
    | 80 | 192 块 | 1.032 | 14 次 / 0.02479e @(−338,294) **新位置** |
    ⇒ 40 与 80 **在同一直段平台**（差异在种子噪声内），但 **40 不引入新跳变源**
    且排水改善更多 ⇒ 取 40。
  - **效果（多口径互证）**：最长轴向直段 **944 → 272 块（0.315× → 0.091×，改善 3.5×）**；
    四方向均衡（160/192/272/260 块）；`runTerrainStripePngProbe` 的 [G] 独立口径
    同向印证（**800 → 360 wu、952 → 208 wu**）；渲染图确认**贯穿性长直线消失、
    边界变为有机曲线**，且**类型区拓扑完全保持**（有界位移 = 拓扑形变，
    非"种子抖动"那种重排格局）。
  - **为何"复用现成机制"而非新方案**：域扭曲**早已存在**（`WARP_AMP` 字段 + 已播种的
    `warpX/warpZ`），只是默认 0；`TectonicField` 已成功使用同一模式
    ⇒ **本次只是把开关打开并标定幅度，没有新写轮子**。
    且它是**唯一"只弯边界、不改格点归属"**的机制 —— 位移有界（≤amp 块，≪400）
    ⇒ 类型图是原图的连续形变，**不触发"改归属 → 排水重排"**（抖动的死因，见下）。
  - **排水哨兵 A/B（关键：抖动的死因是 `border` 1.358→12.772）**：
    amp=40 得 `border.maxSurfaceDelta` **1.845 → 0.764（改善 59%）**、
    `violations=0`、`cycles=0`、`profile.violations=0`、`gateViolations=0`、`status=PASS`
    ⇒ **完全没有出现当年抖动的退化，反而变好**。
  - **其余门禁（amp=40 下）**：`runOreVeinProbe` **ALL PASS**、
    `runCaveBiomeProbe` **ALL PASS**、`runCaveShapeProbe` **ALL PASS**、
    `runChunkBorderProbe` 通过、`runTypeAxisProbe` 三判据 **ALL PASS**。
  - **★ 顺带勘误**：`LandEConformityProbe` **在 amp=0 下本就是 FAIL**
    （实测 10 次 / 0.02251e > 阈值 0.02e）。该探针注释自述
    "保持严格判据并如实报 FAIL，作为后续继续收敛的量化靶子
    （不为了让 CI 变绿而放宽阈值）" ⇒ **它是"已知未达标"的靶子，不是回归**。
    ⚠ **AGENTS 里"ALL PASS"的记载有误，已更正**。采用 40 后为 11 次 / 0.02257e
    （**最坏点同位置、幅度 +0.3%**），如实保留 FAIL。
  - **⚠ 性质与回退**：本改动**改变所有世界的地形大尺度位置**（类型图被形变）
    ⇒ 旧存档/旧预览缓存失效 ⇒ **已 bump `PreviewDisplay.CACHE_SCHEMA_VERSION` 69 → 70**。
    **回退 = 把 `WARP_AMP_DEFAULT` 改回 0.0**（无其它牵连）。
    ⚠ **待用户实机复验**（地形大尺度重排，属必须目检类改动）。
