...
/**
 * SimpleHydrology V15: 精确保留 V7 物理引擎
 * 
 * 与 V7 的唯一不同点：
 * 1. VIEW_SIZE 从 2500 → 6000（大陆变小，海洋更多）
 * 2. 渲染时 3x3 blur → 7x7 blur（河流视觉宽度更大）
 * 3. 粒子数略增（DROPS_PER_ITER 500 → 800）让主干更连贯
 * 
 * 物理引擎：与 V7 完全一致，一字不改
 */
public class SimpleHydrologyV15 {

    static int MAP_SIZE = 512;
    static int ITERATIONS = 150;
    static int DROPS_PER_ITER = 800;
    ...
    static int VIEW_SIZE = 3000;  // 比V7放大20%，大陆稍小，粒子仍有效
    ...
    public SimpleHydrologyV15(int seed) {
        this.seed = seed;
    ...
    void simulate() {
        System.out.println("=== SimpleHydrology V15 (V7物理引擎 + 大视口 + 宽渲染) ===");
    ...
    void renderAndSave(String filePath) throws Exception {
        ...
        // ← 唯一改动2: 7x7 模糊核（V7用的是3x3），让河流渲染更宽
        int BLUR_R = 3; // 半径3 = 7x7
        float[][] blurD = new float[h][w];
        for (int y = BLUR_R; y < h - BLUR_R; y++)
            for (int x = BLUR_R; x < w - BLUR_R; x++) {
                float sum = 0;
                int count = 0;
        ...
        for (int py = 0; py < sz; py++) {
            ...
            for (int px = 0; px < sz; px++) {
                ...
                int r2 = (c1 >> 16) & 0xFF, g2r = (c1 >> 8) & 0xFF, b2r = c1 & 0xFF;
                if (d > p95) {
                    float t = (float) Math.log1p(d - p95) / (float) Math.log1p(maxD - p95 + 1);
                    t = Math.max(0, Math.min(1, t));
                    if (d > p995) {
                        r2 = (int) (10 + t * 20); g2r = (int) (30 + t * 80); b2r = (int) (120 + t * 135);
                    } else if (d > p99) {
                        r2 = (int) (5 + t * 15); g2r = (int) (20 + t * 60); b2r = (int) (80 + t * 100);
                    } else if (d > p98) {
                        r2 = (int) (5 + t * 10); g2r = (int) (15 + t * 40); b2r = (int) (60 + t * 60);
                    } else {
                        r2 = (int) (5 + t * 8); g2r = (int) (15 + t * 30); b2r = (int) (50 + t * 50);
                    }
                }
                ...
                if (c >= 0 && d > p95) {
                    ...
                    t = Math.max(0, Math.min(1, t));
                    if (d > p995) {
                        r3 = (int) (10 + t * 20); g3 = (int) (30 + t * 80); b3 = (int) (120 + t * 135);
                    } else if (d > p99) {
                        r3 = (int) (5 + t * 15); g3 = (int) (20 + t * 60); b3 = (int) (80 + t * 100);
                    } else {
                        r3 = (int) (5 + t * 10); g3 = (int) (15 + t * 40); b3 = (int) (60 + t * 60);
                    }
                }
        ...
        drawLabel(img, "侵蚀后+河流(7x7模糊)", panelW + gap + 10, 2, 0xFFFFFF);
        drawLabel(img, "Minecraft地形+河流", (panelW + gap) * 2 + 10, 2, 0xFFFFFF);
        drawLabel(img, "Discharge热力图", (panelW + gap) * 3 + 10, 2, 0xFFFFFF);
        String info = "Seed=" + seed + " V15 VIEW_SIZE=" + VIEW_SIZE + " Iter=" + ITERATIONS + " Drops=" + (ITERATIONS * DROPS_PER_ITER);
    ...
    int terrainColorSmooth(float h) {
        ...
        float[][] stops = {
            {0.00f, 5, 20, 60}, {0.25f, 20, 60, 120}, {0.35f, 180, 170, 80},
            {0.45f, 100, 150, 50}, {0.55f, 60, 130, 50}, {0.65f, 140, 150, 70},
            {0.75f, 170, 160, 120}, {0.85f, 190, 185, 170}, {1.00f, 230, 230, 240}
        };
        int idx = 0;
        ...
        t = t * t * (3f - 2f * t);
        return rgb(
            (int) (stops[idx][1] + t * (stops[idx + 1][1] - stops[idx][1])),
            (int) (stops[idx][2] + t * (stops[idx + 1][2] - stops[idx][2])),
            (int) (stops[idx][3] + t * (stops[idx + 1][3] - stops[idx][3]))
        );
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        SimpleHydrologyV15 sim = new SimpleHydrologyV15(seed);
        sim.renderAndSave("output/v15_s" + seed + "_" + tag + ".png");
    }
}
