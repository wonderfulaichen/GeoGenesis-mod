...
public class SimpleHydrologyV30 {
    ...
    boolean descendHydroOpt(float[] pos, float[] vel, float[] volSedAge, float baseRadius, float maxRadius, float trackScale, float erosionMul, float depositionMul) {
        ...
        float c_eq;
        if (useVelocityCap) {
            float dischargeCap = (1.0f + entrainment * nodeDischarge) * slope;
            float spd = (float)Math.sqrt(vel[0]*vel[0] + vel[1]*vel[1]);
            float velocityBoost = spd * vol * CAP_FACTOR * 0.03f;
            c_eq = dischargeCap * (1.0f + velocityBoost);
        } else {
            c_eq = (1.0f + entrainment * nodeDischarge) * slope;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("运行优化模式: Opt (ENTRAINMENT=10, SlopeAtten=0.3, ValleySpawn 15%, 4000drops×120iter=48万)");
