...
public class SimpleHydrologyV30 {
    ...
    static float CAP_FACTOR = 10.0f;
    static float MIN_CAP = 0.005f;
    ...
    final float[][] erodeAccum;
    final StandalonePreview terrain;
    final long seed;
    final Random rng;
    final int viewSize;
    float entrainment = ENTRAINMENT;
    float evap = EVAP;
    float slopeAttenuationHeight = 0.0f;
    boolean useVelocityCap = false;
    float depositionRate = DEPOSITION;
    int cascadeInterval = 1;
    ...
    public static class Config {
        ...
        public boolean usePointMode = false;
        public boolean usePyramid = false;
        public float riverSpawnFraction = 0.0f;
        public int riverSpawnWarmup = 5;
        public int updateInterval = 1; // updateDischargeAndMomentum 间隔
        public boolean reversePyramid = false; // 金字塔反向：从小到大

        public Config(String name) { this.name = name; }
    }

    float laplacian(int ix, int iy) {
        if (ix < 1 || ix >= w - 1 || iy < 1 || iy >= h - 1) return 0;
        return height[iy-1][ix] + height[iy+1][ix] + height[iy][ix-1] + height[iy][ix+1] - 4 * height[iy][ix];
    }

    public void runExperiment(Config cfg) {
        if (cfg.usePointMode) {
            float trackScale = 5000.0f / cfg.dropsPerIter;
            int[] valleyCellsX = null;
            int[] valleyCellsY = null;
            float[] valleyCellsCDF = null;
            int valleyCellCount = 0;

            if (cfg.riverSpawnFraction > 0) {
                java.util.List<int[]> cells = new java.util.ArrayList<>();
                java.util.List<Float> weights = new java.util.ArrayList<>();
                for (int y = 2; y < h - 2; y++) {
                    for (int x = 2; x < w - 2; x++) {
                        if (height[y][x] < SEA_LEVEL) continue;
                ...
                valleyCellCount = cells.size();
                if (valleyCellCount > 0) {
                    valleyCellsX = new int[valleyCellCount];
                    valleyCellsY = new int[valleyCellCount];
                    valleyCellsCDF = new float[valleyCellCount];
                    float cumSum = 0;
                    for (int i = 0; i < valleyCellCount; i++) {
                        valleyCellsX[i] = cells.get(i)[0];
                        valleyCellsY[i] = cells.get(i)[1];
                        cumSum += weights.get(i);
                        valleyCellsCDF[i] = cumSum;
                    }
                }
            }
            ...
            for (int iter = 0; iter < cfg.iterations; iter++) {
                resetTracks();
                int valleyDrops = (valleyCellCount > 0)
                    ? (int)(cfg.dropsPerIter * cfg.riverSpawnFraction) : 0;
                int randomDrops = cfg.dropsPerIter - valleyDrops;
                int activeDrops = 0;

                for (int i = 0; i < randomDrops; i++) {
                    float px = rng.nextFloat() * w, py = rng.nextFloat() * h;
                    if (getH((int)px, (int)py) < SEA_LEVEL) { randomDrops++; continue; }
                    activeDrops++;
                ...
                for (int i = 0; i < valleyDrops; i++) {
                    float r = rng.nextFloat() * valleyCellsCDF[valleyCellCount - 1];
                    int lo = 0, hi = valleyCellCount - 1;
                    while (lo < hi) {
                        int mid = (lo + hi) >>> 1;
                        if (valleyCellsCDF[mid] < r) lo = mid + 1; else hi = mid;
                    }
                    int sx = valleyCellsX[lo], sy = valleyCellsY[lo];
            ...
            float[] radii;
            if (cfg.reversePyramid) {
                radii = new float[]{ 0.0f, 1.5f, 3.0f }; // 从小到大
            } else {
                radii = new float[]{ 3.0f, 1.5f, 0.0f }; // 从大到小
            }
            ...
            // Opt模式 - 纯点侵蚀（trackScale仅用于updateDischargeAndMomentum，不应作为radius）
            float trackScale = 5000.0f / cfg.dropsPerIter;
            for (int iter = 0; iter < cfg.iterations; iter++) {
                resetTracks();
                int activeDrops = 0;
                ...
                if (cfg.updateInterval <= 1 || iter % cfg.updateInterval == 0)
                    updateDischargeAndMomentum(trackScale);
            }
        }
    }

    public void runExperimentPhase2(Config cfg) {
        float trackScale = 5000.0f / cfg.dropsPerIter;

        int[] riverCellsX = null;
        int[] riverCellsY = null;
        float[] riverCellsCDF = null;
        ...
        float dischargeThresh = maxD * 0.05f;
        java.util.List<int[]> cells = new java.util.ArrayList<>();
        java.util.List<Float> weights = new java.util.ArrayList<>();
        for (int y = 2; y < h - 2; y++) {
            for (int x = 2; x < w - 2; x++) {
                if (height[y][x] < SEA_LEVEL) continue;
                if (discharge[y][x] < dischargeThresh) continue;
                cells.add(new int[]{x, y});
                weights.add(discharge[y][x]);
            }
        }
        riverCellCount = cells.size();
        if (riverCellCount > 0) {
            riverCellsX = new int[riverCellCount];
            riverCellsY = new int[riverCellCount];
            riverCellsCDF = new float[riverCellCount];
            float cumSum = 0;
            for (int i = 0; i < riverCellCount; i++) {
                riverCellsX[i] = cells.get(i)[0];
                riverCellsY[i] = cells.get(i)[1];
                cumSum += weights.get(i);
                riverCellsCDF[i] = cumSum;
            }
        }

        float riverFraction = 0.3f;

        for (int iter = 0; iter < cfg.iterations; iter++) {
            resetTracks();
            int riverDrops = (riverCellCount > 0)
                ? (int)(cfg.dropsPerIter * riverFraction) : 0;
            int randomDrops = cfg.dropsPerIter - riverDrops;
            int activeDrops = 0;
            ...
            for (int i = 0; i < riverDrops; i++) {
                float r = rng.nextFloat() * riverCellsCDF[riverCellCount - 1];
                int lo = 0, hi = riverCellCount - 1;
                while (lo < hi) {
                    int mid = (lo + hi) >>> 1;
                    if (riverCellsCDF[mid] < r) lo = mid + 1; else hi = mid;
                }
                int sx = riverCellsX[lo], sy = riverCellsY[lo];
                float px = sx + rng.nextFloat(), py = sy + rng.nextFloat();
                if (getH((int)px, (int)py) < SEA_LEVEL) continue;
                activeDrops++;
                float[] pos = {px, py}, vel = {0, 0}, vsa = {1.0f, 0, 0};
    ...
    public void runJacobiExperiment(int cycles, int dropsPerCycle, float erodeScale, float maxErosionPerCycle) {
        float trackScale = 5000.0f / dropsPerCycle;

        for (int cycle = 0; cycle < cycles; cycle++) {
            for (int y = 0; y < h; y++)
                Arrays.fill(erodeAccum[y], 0);

            resetTracks();

            int activeDrops = 0;
            while (activeDrops < dropsPerCycle) {
                float px = rng.nextFloat() * w, py = rng.nextFloat() * h;
                if (getH((int)px, (int)py) < SEA_LEVEL) continue;
                activeDrops++;
                float[] pos = {px, py}, vel = {0, 0}, vsa = {1.0f, 0, 0};
                while (descendPointBatch(pos, vel, vsa)) {}
            }
            ...
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++)
                    if (erodeAccum[y][x] > maxAccum) maxAccum = erodeAccum[y][x];
            ...
            for (int y = 1; y < h - 1; y++) {
                for (int x = 1; x < w - 1; x++) {
                    ...
                    float dNorm = maxD > 0 ? d / maxD : 0;

                    float e = erodeScale * dNorm * (0.1f + slope * 5.0f);
                    e += erodeAccum[y][x] * erodeScale * 0.5f;

                    if (e > maxErosionPerCycle) e = maxErosionPerCycle;
                    if (e > 0) {
                        height[y][x] -= e;
                        if (height[y][x] < 0) height[y][x] = 0;
                    }
                }
            }
            ...
            System.out.println("  Jacobi cycle " + (cycle+1) + "/" + cycles
                + " done, maxAccum=" + String.format("%.4f", maxAccum)
                + " maxD=" + String.format("%.1f", maxD));
        }
    }

    boolean descendPointBatch(float[] pos, float[] vel, float[] volSedAge) {
        ...
        if (age > MAX_AGE || vol < MIN_VOL) {
            if (iy >= 0 && iy < h && ix >= 0 && ix < w)
                erodeAccum[iy][ix] += Math.max(0, sediment);
            return false;
        }
        ...
        if (slopeAttenuationHeight > 0 && delta > 0) {
            float fo = Math.min(1.0f, height[iy][ix] / slopeAttenuationHeight);
            delta *= fo;
        }
        ...
        if (delta > 0) {
            erodeAccum[iy][ix] += delta;
        }

        float effectiveEvap = evap;
        sediment /= (1.0f - effectiveEvap); vol *= (1.0f - effectiveEvap);
    ...
    boolean descendHydroOpt(float[] pos, float[] vel, float[] volSedAge, float baseRadius, float maxRadius, float trackScale, float erosionMul, float depositionMul) {
        ...
        if (cascadeInterval <= 1 || (int)age % cascadeInterval == 0)
            cascade(ix, iy);
        
        float effectiveEvap = evap;
        if (useVelocityCap && slope < 0.005f) {
            float evapFactor = slope / 0.005f;
            effectiveEvap = evap * Math.max(0.1f, evapFactor);
        }
        volSedAge[0] = vol * (1.0f - effectiveEvap); volSedAge[1] = age + 1; volSedAge[2] = sediment / (1.0f - effectiveEvap);
        return true;
    }
    ...
    void applyStreamPowerErosion(float K, float m, float n, int passes) {
        int[] dx8 = {-1, 0, 1, -1, 1, -1, 0, 1};
        int[] dy8 = {-1, -1, -1, 0, 0, 1, 1, 1};
        float[] dist8 = {1.414f, 1.0f, 1.414f, 1.0f, 1.0f, 1.414f, 1.0f, 1.414f};

        float maxDischarge = 0;
        for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++)
                if (discharge[y][x] > maxDischarge) maxDischarge = discharge[y][x];
        float dischargeThresh = maxDischarge * 0.01f;

        int[][] flowDir = new int[h][w];
        float[][] flowAccum = new float[h][w];
        java.util.List<int[]> sorted = new java.util.ArrayList<>(h * w);
        ...
        for (int pass = 0; pass < passes; pass++) {
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    ...
                    for (int d = 0; d < 8; d++) {
                        ...
                        float slope = (height[y][x] - height[ny][nx]) / dist8[d];
                        if (slope > maxSlope) {
                            maxSlope = slope;
                            bestDir = d;
                        }
            ...
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++)
                    if (height[y][x] >= SEA_LEVEL)
                        sorted.add(new int[]{x, y});
            sorted.sort((a, b) -> Float.compare(height[b[1]][b[0]], height[a[1]][a[0]]));

            for (int[] p : sorted) {
                int x = p[0], y = p[1];
                if (flowDir[y][x] < 0) continue;
                int nx = x + dx8[flowDir[y][x]], ny = y + dy8[flowDir[y][x]];
                if (nx >= 0 && nx < w && ny >= 0 && ny < h) {
                    flowAccum[ny][nx] += flowAccum[y][x];
                }
            }

            float[][] deltaMap = new float[h][w];
            for (int y = 1; y < h - 1; y++) {
                for (int x = 1; x < w - 1; x++) {
                    if (height[y][x] < SEA_LEVEL || flowDir[y][x] < 0) continue;
                    float existingD = discharge[y][x];
                    if (existingD < dischargeThresh) continue;

                    int nx = x + dx8[flowDir[y][x]], ny = y + dy8[flowDir[y][x]];
                    if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
                    float slope = (height[y][x] - height[ny][nx]) / dist8[flowDir[y][x]];
                    if (slope <= 0) continue;
                    float A = flowAccum[y][x];
                    float erosion = K * (float)Math.pow(A, m) * (float)Math.pow(slope, n);

                    float dischargeWeight = Math.min(1.0f, existingD / (maxDischarge * 0.1f));
                    erosion *= dischargeWeight;

                    float heightAboveSea = height[y][x] - SEA_LEVEL;
                    if (heightAboveSea < 0.1f) {
                        erosion *= Math.max(0, heightAboveSea / 0.1f);
                    }
                    deltaMap[y][x] -= erosion;
                    float totalSlope = 0;
                    for (int d = 0; d < 8; d++) {
                        int nnx = x + dx8[d], nny = y + dy8[d];
                        if (nnx < 0 || nnx >= w || nny < 0 || nny >= h) continue;
                        if (height[nny][nnx] < height[y][x]) {
                            totalSlope += (height[y][x] - height[nny][nnx]) / dist8[d];
                        }
                    }
                    if (totalSlope > 0) {
                        for (int d = 0; d < 8; d++) {
                            ...
                            if (nnx < 0 || nnx >= w || nny < 0 || nny >= h) continue;
                            if (height[nny][nnx] < height[y][x]) {
                                float s = (height[y][x] - height[nny][nnx]) / dist8[d];
                                deltaMap[nny][nnx] += erosion * (s / totalSlope);
                            }
                        }
                    }
                }
    ...
    static void analyzeRiverSimilarity(SimpleHydrologyV30 simHQ, SimpleHydrologyV30 simOpt, BufferedImage img, int offsetX, int offsetY) {
        ...
        // ====== 断流检测：HQ河流中在Opt缺失的连通分量 ======
        boolean[][] fnMap = new boolean[h][w];
        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++)
            fnMap[y][x] = hqRiver[y][x] && !optRiver[y][x];
        int[] fnComponents = new int[3];
        int[] fnCompSizes = new int[]{0, 0, 0};
        boolean[][] visited = new boolean[h][w];
        int totalFnPixels = 0, smallFnPixels = 0;
        java.util.Queue<int[]> bfs = new java.util.LinkedList<>();
        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++) {
            if (!fnMap[y][x] || visited[y][x]) continue;
            bfs.clear();
            bfs.add(new int[]{x, y});
            visited[y][x] = true;
            int compSize = 0;
            while (!bfs.isEmpty()) {
                ...
                for (int dy = -1; dy <= 1; dy++) for (int dx = -1; dx <= 1; dx++) {
                    ...
                    if (nx >= 0 && nx < w && ny >= 0 && ny < h && fnMap[ny][nx] && !visited[ny][nx]) {
                        visited[ny][nx] = true;
                        bfs.add(new int[]{nx, ny});
                    }
                }
            }
            totalFnPixels += compSize;
            if (compSize < 10) smallFnPixels += compSize;
            if (compSize >= 50) fnComponents[2]++;
            else if (compSize >= 10) fnComponents[1]++;
            else fnComponents[0]++;
        }
        System.out.println("");
        System.out.printf("  ===== 断流检测 (HQ有但Opt缺失的河道段) =====\n");
        System.out.printf("  缺失总像素:   %d (占HQ河流 %.1f%%)\n", totalFnPixels, hqRiverPixels > 0 ? (float)totalFnPixels/hqRiverPixels*100 : 0);
        System.out.printf("  小碎片(<10px): %d段, %d px\n", fnComponents[0], smallFnPixels);
        System.out.printf("  中断段(10-49px): %d段\n", fnComponents[1]);
        System.out.printf("  大断流(>=50px):  %d段 ← 关键指标\n", fnComponents[2]);
        System.out.printf("  ================================\n");
        System.out.println("");
    ...
    public static void main(String[] args) throws Exception {
        ...
        simHQ.entrainment = 6.0f;
        simHQ.simulate();

        System.out.println("运行优化模式: Opt v9 (基准, 48万粒子)");
        SimpleHydrologyV30 simOpt = new SimpleHydrologyV30(seed, VIEW_SIZE_A);
        simOpt.entrainment = 10.0f;
        simOpt.slopeAttenuationHeight = 0.3f;
        simOpt.useVelocityCap = false;
        simOpt.evap = EVAP;
        Config cfg = new Config("Opt Point");
        cfg.iterations = 120;
        cfg.dropsPerIter = 4000;
        cfg.usePointMode = true;
        cfg.usePyramid = false;
        cfg.riverSpawnFraction = 0.15f;
        simOpt.runExperiment(cfg);

        System.out.println("运行组合优化: Cascade5 + Update10 (48万粒子)");
        SimpleHydrologyV30 simC5U10 = new SimpleHydrologyV30(seed, VIEW_SIZE_A);
        simC5U10.entrainment = 10.0f;
        simC5U10.slopeAttenuationHeight = 0.3f;
        simC5U10.useVelocityCap = false;
        simC5U10.evap = EVAP;
        simC5U10.cascadeInterval = 5;
        Config cfgC5U10 = new Config("C5+U10");
        cfgC5U10.iterations = 120;
        cfgC5U10.dropsPerIter = 4000;
        cfgC5U10.usePointMode = true;
        cfgC5U10.usePyramid = false;
        cfgC5U10.riverSpawnFraction = 0.15f;
        cfgC5U10.updateInterval = 10;
        simC5U10.runExperiment(cfgC5U10);

        System.out.println("运行组合优化: Cascade10 + Update10 (48万粒子)");
        SimpleHydrologyV30 simC10U10 = new SimpleHydrologyV30(seed, VIEW_SIZE_A);
        simC10U10.entrainment = 10.0f;
        simC10U10.slopeAttenuationHeight = 0.3f;
        simC10U10.useVelocityCap = false;
        simC10U10.evap = EVAP;
        simC10U10.cascadeInterval = 10;
        Config cfgC10U10 = new Config("C10+U10");
        cfgC10U10.iterations = 120;
        cfgC10U10.dropsPerIter = 4000;
        cfgC10U10.usePointMode = true;
        cfgC10U10.usePyramid = false;
        cfgC10U10.riverSpawnFraction = 0.15f;
        cfgC10U10.updateInterval = 10;
        simC10U10.runExperiment(cfgC10U10);

        // 多种子验证
        long seed2 = 54321;
        System.out.println("\n=== 多种子验证 (seed=" + seed2 + ") ===");

        System.out.println("运行种子2基准: HQ Baseline");
        SimpleHydrologyV30 simHQ2 = new SimpleHydrologyV30(seed2, VIEW_SIZE_A);
        simHQ2.entrainment = 6.0f;
        simHQ2.simulate();

        System.out.println("运行种子2优化: Opt v9 (基准)");
        SimpleHydrologyV30 simOpt2 = new SimpleHydrologyV30(seed2, VIEW_SIZE_A);
        simOpt2.entrainment = 10.0f;
        simOpt2.slopeAttenuationHeight = 0.3f;
        simOpt2.useVelocityCap = false;
        simOpt2.evap = EVAP;
        Config cfg2 = new Config("Opt Point");
        cfg2.iterations = 120;
        cfg2.dropsPerIter = 4000;
        cfg2.usePointMode = true;
        cfg2.usePyramid = false;
        cfg2.riverSpawnFraction = 0.15f;
        simOpt2.runExperiment(cfg2);

        System.out.println("运行种子2组合: Cascade5 + Update10");
        SimpleHydrologyV30 simC5U10_2 = new SimpleHydrologyV30(seed2, VIEW_SIZE_A);
        simC5U10_2.entrainment = 10.0f;
        simC5U10_2.slopeAttenuationHeight = 0.3f;
        simC5U10_2.useVelocityCap = false;
        simC5U10_2.evap = EVAP;
        simC5U10_2.cascadeInterval = 5;
        Config cfgC5U10_2 = new Config("C5+U10");
        cfgC5U10_2.iterations = 120;
        cfgC5U10_2.dropsPerIter = 4000;
        cfgC5U10_2.usePointMode = true;
        cfgC5U10_2.usePyramid = false;
        cfgC5U10_2.riverSpawnFraction = 0.15f;
        cfgC5U10_2.updateInterval = 10;
        simC5U10_2.runExperiment(cfgC5U10_2);

        System.out.println("运行种子2组合: Cascade10 + Update10");
        SimpleHydrologyV30 simC10U10_2 = new SimpleHydrologyV30(seed2, VIEW_SIZE_A);
        simC10U10_2.entrainment = 10.0f;
        simC10U10_2.slopeAttenuationHeight = 0.3f;
        simC10U10_2.useVelocityCap = false;
        simC10U10_2.evap = EVAP;
        simC10U10_2.cascadeInterval = 10;
        Config cfgC10U10_2 = new Config("C10+U10");
        cfgC10U10_2.iterations = 120;
        cfgC10U10_2.dropsPerIter = 4000;
        cfgC10U10_2.usePointMode = true;
        cfgC10U10_2.usePyramid = false;
        cfgC10U10_2.riverSpawnFraction = 0.15f;
        cfgC10U10_2.updateInterval = 10;
        simC10U10_2.runExperiment(cfgC10U10_2);

        // 创建大图：seed1 三列 + seed2 三列
        int margin = 30;
        int canvasW = MAP_SIZE * 6 + margin * 5;
        int canvasH = MAP_SIZE * 2 + 120;
        ...
        simC5U10.renderOnto(img, MAP_SIZE + margin, 0);
        simC10U10.renderOnto(img, MAP_SIZE * 2 + margin * 2, 0);
        simHQ2.renderOnto(img, MAP_SIZE * 3 + margin * 3, 0);
        simC5U10_2.renderOnto(img, MAP_SIZE * 4 + margin * 4, 0);
        simC10U10_2.renderOnto(img, MAP_SIZE * 5 + margin * 5, 0);

        g.setColor(Color.WHITE);
        g.setFont(new Font("Arial", Font.BOLD, 22));
        g.drawString("HQ (seed1)", 20, 35);
        g.drawString("C5+U10 (seed1)", MAP_SIZE + margin + 20, 35);
        g.drawString("C10+U10 (seed1)", MAP_SIZE * 2 + margin * 2 + 20, 35);
        g.drawString("HQ (seed2)", MAP_SIZE * 3 + margin * 3 + 20, 35);
        g.drawString("C5+U10 (seed2)", MAP_SIZE * 4 + margin * 4 + 20, 35);
        g.drawString("C10+U10 (seed2)", MAP_SIZE * 5 + margin * 5 + 20, 35);

        g.setFont(new Font("Arial", Font.BOLD, 16));
        g.drawString("=== Seed 1 ===", 20, 55);
        g.drawString("=== Seed 2 ===", MAP_SIZE * 3 + margin * 3 + 20, 55);
        ...
        System.out.println("\n=== HQ vs Cascade5+Update10 ===");
        analyzeRiverSimilarity(simHQ, simC5U10, img, MAP_SIZE + margin, 0);
        System.out.println("\n=== HQ vs Cascade10+Update10 ===");
        analyzeRiverSimilarity(simHQ, simC10U10, img, MAP_SIZE * 2 + margin * 2, 0);

        System.out.println("\n========== SEED 2 RESULTS ==========");
        System.out.println("\n=== HQ vs Opt v9 (基准, seed2) ===");
        analyzeRiverSimilarity(simHQ2, simOpt2, img, MAP_SIZE * 3 + margin * 3, 0);
        System.out.println("\n=== HQ vs Cascade5+Update10 (seed2) ===");
        analyzeRiverSimilarity(simHQ2, simC5U10_2, img, MAP_SIZE * 4 + margin * 4, 0);
        System.out.println("\n=== HQ vs Cascade10+Update10 (seed2) ===");
        analyzeRiverSimilarity(simHQ2, simC10U10_2, img, MAP_SIZE * 5 + margin * 5, 0);
