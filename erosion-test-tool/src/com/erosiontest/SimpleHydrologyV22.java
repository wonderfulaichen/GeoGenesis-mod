...
/**
 * SimpleHydrology V22: 双列对比（细节 + 概览）
 * 
 * 物理引擎与 V20 完全一致。
 * 渲染改为 2 列：
 * - 左列：1x 原比例尺（细节）
 * - 右列：0.33x 缩小比例尺（全局概览）
 */
public class SimpleHydrologyV22 {
    ...
    public SimpleHydrologyV22(int seed) {
        this.seed = seed;
    ...
    public void simulate() {
        System.out.println("=== SimpleHydrology V22 (双列: 1x细节 + 0.33x概览) ===");
    ...
    public void save(String path) throws Exception {
        ...
        // 双列：左列 1x，右列 min(0.25x, 0.25x)
        float rightScale = 0.25f;
        int rightW = (int)(w * rightScale);
        int rightH = (int)(h * rightScale);
        int gap = 4;

        BufferedImage img = new BufferedImage(w + gap + rightW, h, BufferedImage.TYPE_INT_RGB);
        ...
        // 填充间隙
        for (int py = 0; py < h; py++)
            for (int px = w; px < w + gap; px++)
                img.setRGB(px, py, 0);

        // 右侧（0.25x 缩略图）
        for (int py = 0; py < rightH; py++) {
            for (int px = 0; px < rightW; px++) {
                int sy = (int)(py / rightScale);
                int sx = (int)(px / rightScale);
                ...
                if (hVal < SEA_LEVEL) {
                    ...
                    t = Math.max(0, Math.min(1, t));
                    if (rawD > p995)
                        color = rgb((int)(30+t*40), (int)(80+t*100), (int)(200+t*55));
                    else if (rawD > p99)
                        color = rgb(20, (int)(60+t*40), (int)(160+t*60));
                    else
                        color = rgb(10, (int)(40+t*30), (int)(100+t*50));
                } else {
                    color = 0x080c06; // 暗绿陆地
                }
                img.setRGB(w + gap + px, py, color);
            }
        }
        ...
        g.drawString("V22 双列 | Seed="+seed+" | maxD="+fmt1(maxD), 10, 14);
        g.setFont(new java.awt.Font("Monospaced", java.awt.Font.PLAIN, 12));
        g.drawString("1:1 比例 (VIEW_SIZE="+VIEW_SIZE+")", 10, h-8);
        g.drawString("1:"+(int)(1/rightScale)+" 概览", w+gap+10, h-8);
        g.dispose();
    ...
    public static void main(String[] args) throws Exception {
        new SimpleHydrologyV22(12345).save("output/v22_s12345.png");
    }
}
