...
### 未决（勿擅自改）
- ~~**金属矿**~~ ✅ **已解决（2026-09-16）**：新增总开关 `[Ores] oreVeinsEnabled`（默认 `true`）。
  `false` = **关闭自研地质矿脉 ⇒ 只剩原版矿（纯原版体验）**。
...
- **已知遗留（两项均已量化并给出结论，均**不改**）**：
  ...
  2. **类型场 Voronoi 轴对齐直段 → 已量化 → 试修 → 回退**：
     - **量化**：新增 `runTerrainStripePngProbe` 的 **[G] 段**（指标 = 最长轴对齐直段 / 采样边长 + 水平:竖直方向比）。窗口 `(-1500,1500)` 实测最长水平直段 **560wu（0.19×）** 且**正好落在网格中垂线 `z=3204`**；另一窗口方向比 2.35 属**窗口地理取样**而非系统偏差（对照窗口 1.10）。
       —— **⚠️ 更正（2026-09-16）："属窗口地理取样"这一判断被数据推翻** ——
       相位扫描显示**跨相位 sd 仅 0.08**、而**跨种子 sd 高达 0.40~0.53**（差 5~6 倍）
       ⇒ 读数差异**主要来自"哪个世界"（种子），不是窗口相位**。该读数的正确描述是
       **"属种子差异"**。（详见本条末尾的补充。）
     ...
     - **★ 2026-09-16 补充：探针已升级为【多种子×多相位】，并纠正两条旧判断**
       （`runTypeAxisProbe`，纯诊断源集、不影响产出）：
       - **为何升级**：初版只跑单种子单窗口，而实测读数**随种子在 0.28~3.20 漂移**
         （跨种子 sd 中位 **0.53**，n=64）⇒ **单点读数根本不能用于决策**。
       - **★ 方差主要来自【种子】而非【相位】**：跨种子 sd **0.396~0.531** vs 跨相位 sd
         **0.080~0.087** ⇒ 上文"某窗口 2.35 属窗口地理取样"**已更正为"属种子差异"**。
       - **★ 判据盲区（最重要）**：`方向比` 只测"轴向 vs 对角"的**各向异性**；
         方形格点下 **0°/45°/90° 三个方向都又长又直** ⇒ 两边都长、比值≈1
         ⇒ **判据通过，而边界照样是一条条长直线**。实测：方向比中位 **1.11**（"达标" ≤1.5）
         而最长轴向直段 **中位 0.307×窗口 ≈ 920 块**（历史记录 560 块）
         ⇒ **已新增判据3「绝对直段长」补盲区**（相对量 + 绝对量互补，
         **只看比例判据 = 会被 45° 直段骗过**）。
       - **★ 判据设计的教训（两面都栽过）**：拿单点数值框**重尾分布**，
         **太松**（初版单样本 ≤2.6 ⇒ 换种子 3.20 即漏报）与
         **太严**（第二版 n 样本 max ≤2.6 ⇒ 8 种子集 3.20 即**误报**，**代码一字未改**）
         是同一个病 ⇒ 判据改用**分布统计量**：判据1 中位 ≤1.5 · 判据2 **p90** ≤2.6 ·
         判据3 直段中位 ≤0.40×窗口；**`max` 只报告、不判定**。
       - **锚定基线（可复现）**：n=64 中位 **1.11** / p90 **1.87** / 直段中位 **0.307×**；
         n=20 为 **1.16 / 1.60 / 0.315×** ⇒ 余量均 **27~39%**。
       - **指纹**：`seed=777/42` 在 4 相位下方向比**精确不变** ⇒ 最长直段长度由
         **格点几何**决定、与窗口切割无关 ⇒ **"相位不变"即格点伪影的判据**。
       - **★ Step 0 闭环（同日续查）：可见性已确认 ⇒ 值得修**。
         `build/stripe/type_dominant.png`（seed=5436529513624899584、size=1000、step=4）
         **肉眼可见贯穿性的竖直直线边界**与长水平直边，与 [G] 报出的 800 / 952 wu 一致。
         - **★ 勘误（又一类"搜不到≠不存在"）**：本轮曾断言"[G] 探针不在仓库里"——**错了**，
           它就在 `TerrainStripePngProbe` 的 **[G] 段**（**AGENTS 原文其实写明了**：
           "新增 `runTerrainStripePngProbe` 的 [G] 段"）。误判源于按字面搜
           `560 / 水平直段` 未命中 —— 它打印的是"最长**水平走向**直段 = 800 wu"，
           **用词不同**。⚠ 与 README 那轮"关键词没命中就判过时"是同类错误，
           **两条合起来说明：本项目的"检索式推断"必须回源头核实。**
         - **★ [G] 判据自相矛盾（应修正）**：它打印"最长竖直走向直段 = **952 wu（0.32×）**"，
           结论却是"最值 **≪1.00** … ⇒ 类型边界无轴对齐倾向" ⇒
           **0.32 不是"≪1.00"，该结论文本无法由它自己的数字支撑**；
           且 [G] **只测轴向**（水平/竖直走向）、**对 45° 对角直段完全盲**。
         - **★ 交叉验证（指标可信）**：本探针 0.307~0.315× vs [G] 0.27/0.32×
           ⇒ **两套独立实现同一量级**。
         - **仍未做**：方案 B 的关键前提（"归属逐点不变"）**尚未证明**；实机目视建议补。
