...
| 要求 | 现状 | 证据 |
|---|---|---|
| **模拟现实的水文** | ⚠️ 判水逻辑正确，但**架构脆弱**（见下） | 河分歧 0.00%、湖真漏灌 0% |
| **适应无限世界** | ✅ 逐格闭式 + 网格原点对齐 | 接缝线不一致 100% → 0% |
| **性能不错** | ✅ **7.92 ms/chunk** | `runChunkLoadPerfProbe` |

## 本轮实测（6 个假设全被否决 —— 说明"我猜的问题"多半不存在）

| 假设 | 实测 | 判决 |
|---|---|---|
| 判水有 32% 漏灌 | 分歧 **0.00%** | ❌ 判据错 |
| 性能是瓶颈 | **7.92 ms/chunk** | ❌ 不是 |
| `W` 可替代 `spill` | σ=4.60、仅 12% \|差\|<1 | ❌ 语义不同 |
| 湖节点多可合并 | **仅 5 个** | ❌ 不成立 |
| 加参数可抬河谷 | 仅变 **0.5%** | ❌ 空转（已撤销） |
| "低于水面却干"是缺陷 | **100% 可排水** | ❌ **概念本身错** |

## ★ 真正的问题不是"某处坏了"，而是【架构脆弱】

```
⑤ 归属竞争（sampleRegion）  ：按【距离】决定这格归湖还是归河
⑥ 分支决策（carveColumn）   ：按【有无湖命中】决定走湖分支还是河分支
```

**两套判据、耦合但不同源** ⇒ 我改 ⑤，⑥ 就跳变：

- 放宽湖认领（修湖岸直线）⇒ **203 列河道被湖分支抢走、河流不再下切**（已修复）
- **这就是"三套基准"病的同一个根**，也正是你要重构掉的东西

## ⇒ 重构的第一刀（本计划的起点）

**把 ⑤ 与 ⑥ 统一到【同一套判据】** —— 对齐参考实现的单一规则：

```
Farseek: isStreamBed = (maxFloorLevel < surfaceLevel)          ← 一套判据
RTF    : isSubMerged = dist < zone1Radius && … && h < water    ← 一套判据
我们    : ⑤ 按距离竞争 + ⑥ 按有无湖命中                        ← 两套 ⇒ 耦合跳变
```

**做法**：让 `sampleRegion`（⑤）也遵守【河道内 → 河优先】，与 ⑥ 同源。
**验收**：`runRiverLakeStealProbe` 的"河+湖双命中"大幅下降，且雕刻结果不变。

## 之后的顺序（★ 进度已更新）

```
第 1 刀  ⑤⑥ 判据统一                ✅ 已完成（2c3e15b）
        湖命中不再发向河道内 ⇒ 网络层"河道内被抢" 203 → 0；
        水体 31327 与统一前完全相同（零行为变更）；门禁 ALL PASS。

第 2 刀  湖域过大（形貌，非性能）      ✅ 已完成（c8f6860）
        定位（§1.18）：2127 列"河道外 + 是陆地 + 在河谷带"中，1497 列（70.4%）
        被湖分支接管 ⇒ 这些陆地河谷【完全没被雕】。
        判决实验（runLakeDomainSweepProbe 扫容差）：
          48wu 水体8228/失水0/抑制2168 → ★12wu 8207/21/1209 → 6wu 7948/280/782
        ⇒ 采用 12wu。效果：实际有水 −0.3%、"该有水却干" 30.9%→16.7%、
          抑制河谷 −22%；门禁 ALL PASS；出图湖岸自然、无硬边。

第 3 刀  水位来源收敛                 ⏳ 降级
        实测：W 与 spill 语义不同（σ=4.60），且河湖水位差 p50=−10.7 主要是
        【上游支流 vs 湖面】的自然落差、非缺陷 ⇒ 无需强行收敛。
（C3 河谷剖面：实测判定【不需要做】，见 §1.17.6）
```

**详细论证与全部实测数字见 §1~§2；任务树见 §1.14。**

---

## 1.18 ★★★★ 第二刀的前置实测：湖域【抑制河雕刻】（1497 列）

> 探针：`runRiverLakeLevelGapProbe`（新增，零生产改动）。

### 1.18.1 实测（seed 9139912035078620160 @ 块(-377,-335)±128，步长 2）

```
[1] 命中构成（采样 16641 列）
    河湖双命中 = 3773（22.7%）；仅河 = 203；仅湖 = 12376；无命中 = 289

[2] 河水面 − 湖水位（n=3773）
    p10=−11.884  p50=−10.724  p90=−2.853  最小=−11.884  最大=+13.677
    ★ |差|>1 块 = 3773（100.0%）；|差|>4 块 = 3588（95.1%）

[3] ★★ 湖域抑制河雕刻（河道外 + 地形高于湖面 + 有河命中）
    符合条件列数 = 2127
    ★ 被湖分支接管（lakeNode != null ⇒ 湖不挖地） = 1497（70.4%）
    ✅ 走河分支且已下切                            = 0（0.0%）
    ○ 走河分支但无需下切（original ≤ bedTarget）  = 630（29.6%）
```

### 1.18.2 判决

**⇒ 2127 列符合"河道外 + 高于湖面（是陆地）+ 在河的谷壁带内"，其中 1497 列（70.4%）
被湖分支接管 ⇒ 湖分支 `carved = original` ⇒ 【这些陆地河谷完全没被雕】。**

**⇒ 这是"湖域过大"的【形貌代价】（此前我只量了它的性能代价，判为"可忽略"——
性能确实可忽略，但形貌不行）。**
...
| | RTF | 我们 |
|---|---|---|
| 湖区内的地形 | **雕刻**（湖 = 更宽的河床，`zone1Radius *= lakeMultiplier`） | **不雕**（`carved = original`，靠自然盆地） |

**⇒ 我们不雕湖底（更自然），但连带把【穿过湖盆的河谷】也一起放弃了 ⇒ 这是副作用。**

### 1.18.3 修法候选（已判决：采用 B）

| 方案 | 含义 | 结论 |
|---|---|---|
| A. 湖分支也做谷壁塑形 | 保留"湖底不雕"，但河谷带仍按河分支塑形 | 未采用（逻辑复杂，且 B 已够） |
| **B. 收窄湖域** | 只让湖认领真正的水体+岸线附近 | ✅ **采用（c8f6860）** |
| C. 维持现状 | 承认湖盆内河谷不塑形 | 否决（代价已量化） |

**⚠ 方案 B 的回归风险**：湖的水边由落块侧 `height < spill` 等高线定，
但**只有拿到湖命中的列**才走湖分支；域一收窄，域外列落回河分支 ⇒ 不灌 ⇒
域边界可能重新成为硬边。**⇒ 必须扫容差量化。**

### 1.18.4 判决实验：`runLakeDomainSweepProbe`（新增，零生产改动）

扫 `RiverLineNetwork.domainToleranceOverride`（默认 −1 = 用生产值）：

```
容差wu   水体列    ★失水     ★多出    抑制河谷
0        6814      1414      0        333
6        7948      280       0        782
★12      8207      21        0        1209      ← 采用（失水 0.26%，抑制 −44%）
24       8207      21        0        1687
48       8228      0         0        2168      ← 原生产值
96       8383      0         155      2168
```

**⇒ 12~24wu 是甜点；再收窄失水陡增（6wu 已失 280，0wu 失 1414）。**
**⇒ 判决：采用 12wu。**

### 1.18.5 落地效果（T=48 → T=12）

| 指标 | 前 | 后 |
|---|---|---|
| 实际有水 | 31327 | **31225（−0.3%）** |
| 该有水 | 45307 | **37496（−17%，语义收紧）** |
| "该有水却干" | 13980（30.9%） | **6271（16.7%）** |
| 抑制河谷 | 1497 | **1168（−22%）** |
| 河湖分歧 | 0.00% | **0.00%** |
| 门禁 | — | **ALL PASS** |
| 出图 | — | 湖岸自然、无直线、无硬边 |
...
计划注释（2026-09-09）曾写"外扩太小（半格 12wu）⇒ 用户实测『水边没贴到地形/
水面包不住』"。**但同一段注释自己给出了结论**：

> "真正的根因是 floodOOB（认领域 < BFS 搜索窗导致整湖被弃），已在 LakeNode.computeFlood 修复。"

**⇒ 12wu 是在 floodOOB 未修时被判失败的，修复后从未复测。** 本次实测（floodOOB 已修）：
12wu 相对 48wu 只少 21 列水体（0.26%）。

**⚠ 但实机才是终审。若出现"水边没贴到地形/水面包不住"，回退：**
`params.gridCell() * 0.5` → `params.gridCell() * 2.0`（一行）。
...
## 1.17 ★★★★ 实测（2026-09-19）：湖节点【本来就很少】—— T2.4 合并思路否决

> 探针：`runLakeNodeCensusProbe`（新增，零生产改动）。

### 1.17.1 实测（seed 9139912035078620160，region(-1,-1) 起 3×3）

```
[1] 湖节点总数 = 5（9 个 region）          ← ★ 极少
[2] 洼地格数分布（总格数 43）
    2格      1（20.0%）
    3-4格    1（20.0%）
    5-8格    2（40.0%）
    17-32格  1（20.0%）
    ★ 碎湖（≤2 格）= 1（20.0%）
[3] 湖面水位：172.105 / 189.051 / 211.964  ← 各不相同
[4] 合并潜力（水位差 ≤0.5 块）= 0（0.0%）  ← ★ 无可合并
```

### 1.17.2 判决：**T2.4（湖节点合并）思路不成立**

**⇒ 只有 5 个湖节点、无可合并、碎湖仅 1 个 ⇒ 湖节点【不是】成本来源。**

**⇒ 关键澄清：**
| 量 | 值 | 含义 |
|---|---|---|
| 湖**域**列数 | 14338 | **域覆盖面积大**（`inDomain` 外扩 4×gridCell） |
| 湖**节点**数 | **5** | 每个节点一次 `computeFlood` ⇒ **仅 5 次 BFS** |

**⇒ 二者完全不同。"湖域空转 46.5%" 是【域大面积】导致的判定空转，
而 BFS 次数【只有 5 次】⇒ 性能成本可忽略（与 §T4.1 的 7.92 ms/chunk 一致）。**

### 1.17.3 ⇒ 重新定位：真问题是什么

**性能（你的要求之一）：✅ 已达标**（7.92 ms/chunk，湖 BFS 仅 5 次）。
**判水：✅ 已正确**（§1.13.5a）。
**跨区一致性：✅ 已修**（格点对齐，接缝 100%→0%）。

**⇒ 剩下的、真正影响"模拟现实"的是：**

1. **`C3` 河谷剖面**（唯一还缺的核心，§1.12.1 C3）
   —— 实测：需抬升 7533 列，p50=2.33 / p90=13.43 / max=47.21，**99.4% 成带** ⇒ 方向已确认可做；
2. **湖域过大**（14338 列 vs 有水 7664）⇒ 虽不耗 CPU，但会造成
   **"湖域内却无水"的观感**（若实机能看出来）⇒ 属**视觉/物理合理性**，非性能。

**⇒ 因此 T2 的剩余价值 = 物理合理性（让湖域 ≈ 实际淹水区），不是性能。**

### 1.17.4 ★★★ C3 落地评估（同探针 [8] 段，落生产代码前的最后把关）

```
[8] C3 落地评估（抬升目标 = surf + 1.0）
    需抬升【干列】 = 7533（占带水位列 69.8%）；已有水列 = 3256（★ 完全不动）
    抬升量：p50=2.332  p90=13.434  max=47.214 块
    ★ 抬升后这些列【由可能变水 变回 确定干】= 7533 列
```

**⇒ 好消息：C3 只抬【干列】，3256 个已有水列【完全不动】
⇒ 不会淹地、不会改变现有水面 ⇒ 无"改坏水"的风险。**

**⇒ 但改动面很大：69.8% 的带水位干列要抬，p90 = 13.4 块、max = 47.2 块。**

**⚠ 这是"先量后改"应当停下来的信号**：
- 抬升 13~47 块会把河谷两侧**整体抬高**，视觉上可能是"河谷变深/两侧变高台"；
- 这个改动**必须用眼睛判断**（参考项目里，`minBankHeight` 是**可调参数**：
  RTF 主河 `minBankHeight=2`、支流 `1`；Farseek `outletSlopes` 正数从 1 递增到 5）。
- **⇒ 不应盲改。应先做一个【参数化 + 默认关闭】的实现，靠实机调参决定取值。**

**⇒ 落地方式（建议）**：
```
新增 RiverLineParams.minBankHeight（默认 0 = 关闭，逐位一致）
打开后：河谷带雕刻目标 = max(bedTarget, waterSurface + minBankHeight)
⇒ 可在实机直接调 0 / 1 / 2 / 3 对比，选视觉最自然的取值
```

### 1.17.5 ★★★★★ 实测：第一版 C3 实现【无效】，已撤销（务必勿重犯）

**曾做**：在 `HydrologyBlockCarver` 的干地下界 `dryFloor` 上叠加 `minBankHeight`，
期望让"河谷两侧高于水面"。

**★ 实测（A/B：`minBankHeight` 0 → 1）**：

```
打开前：需抬升干列 7533（占带水位列 69.8%），p50=2.332
打开后：需抬升干列 7495（占带水位列 69.7%），p50=2.355
⇒ 只变了 38 列（0.5%）—— 【空转】
```

**★ 根因**：`cut = Math.max(0.0, original − bedTarget)` ⇒ **`dryFloor` 只【阻止挖低】，
不会【抬升】地形**。原地形低于水面的列 ⇒ `cutNeeded = 0` ⇒ `carved = original`（纹丝不动）。

**⇒ 已撤销**（`RiverLineParams.minBankHeight` 与 carver 改动一并移除，留下警示注释）。

**★★★ 由此得出的重要结论**：

> **真正的 C3 需要【抬升地形】，而现有铁律是「只下挖」（`carved ≤ original`）。**
> **⇒ 这不再是"加个参数"，而是【设计决策】：要不要打破"只下挖"？**

| 选项 | 含义 | 风险 |
|---|---|---|
| **A. 打破只下挖** | 允许抬升河谷两侧到水面之上 | 可能造出"悬空平台"；破坏侵蚀一致性 |
| **B. 不改地形，改水面** | 若那些列地形低于水面，说明**河线水面偏高** ⇒ 修水位（C1） | 需先证明"水面偏高"而非"地形偏低" |
| **C. 维持现状** | 承认这些列"低于水面但干" | 若实机看不出异常，可接受 |

**⇒ 判据（下一步该量什么）**：
若那些列的 `surf` 来自**上游河段**（纵向剖面）⇒ 是 B（水面传导错误）；
若来自**本段** ⇒ 是 A（地形该抬）。
**⇒ 必须量"漏灌列的 surf 来自哪一段"才能定。**

**★ 教训（同类第四次）**：
**"把约束加到 target 上"≠"改变结果"—— 必须验证【实际 carved】是否变。**
我这次加了参数、写了注释，却没验证它有没有真的生效，差点留下一个误导性的死参数。

### 1.17.6 ★★★★★★ 决定性判别：C3 【不需要做】—— "漏灌"概念本身不成立

> 探针：`runUnifiedCriterionProbe` [9] 段（新增）。
> **判据的思想来源**：水在 S 处会【往低处流】。某列 `h < S` 但**附近有更低的地**
> ⇒ 水从那里排走 ⇒ **该列不该积水**。只有【排不出去】的列才是真缺陷。
> （这与 RTF `isSubMerged` / 湖分支 `inFlood` 是同一思想：**水的存在需要"无出口/连通"**。）

```
[9] 本质判别：'低于水面却干'的列 = 3585 个
    半径 1 内有更低邻居（可排水）: 3585/3585 = 100.0%
    半径 2 内                    : 3585/3585 = 100.0%
    半径 4 内                    : 3585/3585 = 100.0%
    半径 8 内                    : 3585/3585 = 100.0%
    ★ 8 邻内无更低者（局部洼地，排不出去）: 0（0.0%）
```

**⇒ ★★★ 判决：那些"低于水面却干"的列【100% 都能排水】、一个"排不出去"的都没有
⇒ 水会从更低处流走 ⇒ 现状"干"是【物理正确的】。**

**⇒ 我这几轮反复围绕的"漏灌 22.5% / 32.81%"概念【本身就不成立】** ——
"低于水面"**不等于**"该有水"；**还必须"排不出去"**。

**⇒ 因此 `C3`（河谷剖面 / 抬升河谷两侧）【不需要做】。**
**⇒ §1.13.5b、§1.17.4、§1.17.5 中由"漏灌"派生的一切推论（含 C3 落地评估）全部作废。**

**★★★ 这一轮的方法论总账（6 次否决）：**

| # | 我的假设 | 实测 | 判决 |
|---|---|---|---|
| 1 | 判水有 32% 漏灌 | 分歧 **0.00%** | ❌ 判据错 |
| 2 | 性能是瓶颈 | **7.92 ms/chunk** | ❌ 不是 |
| 3 | `W` 可替代 `spill` | σ=4.60、仅 12% \|差\|<1 | ❌ 语义不同 |
| 4 | 湖节点多可合并 | **仅 5 个、合并潜力 0** | ❌ 不成立 |
| 5 | 加参数可抬河谷（C3） | 0→1 仅变 **0.5%** | ❌ 空转 |
| 6 | **"低于水面却干"是缺陷** | **100% 可排水** | ❌ **概念本身错** |

**★ 共同根因**：我**先有"这里有问题"的直觉，再去找证据** —— 而不是
**先按参考实现定义"什么才算问题"，再测量**。
真正的判据（RTF `isSubMerged` 三条件）我早先读过、写进 §1.2，却在测量时一次次漏用。

**⇒ 铁律（写给后来人）**：
> **任何"缺陷"都必须先写下来自参考实现的【判据】，再测量；
> 且判据必须包含【为什么这算缺陷】的物理理由（如"排不出去"），而不只是数值比较。**
...
T2 [P0] 湖去对象化（C7）—— 从"独立系统"到"节点属性/乘数"
  ...
  T2.3  ✅ **已定位真实成本**（读代码确认）：`computeFlood` 已有缓存
        （`if (floodX != null) return floodOOB`）⇒ **每湖只算一次**
        ⇒ 湖域空转的代价 = **每个湖节点一次 BFS + O(1) 索引构建**
        ⇒ ★ **T2 的正解 = 减少【湖节点数量】，而非收窄域**
  T2.4  ⏳ 实现：湖节点去重/合并（相邻同水位的 LakeNode 合成一个）
        对齐 RTF 范式：湖 = 河床半径乘数（`zone1Radius *= lakeMultiplier`），非独立分支
  T2.5  ⏳ 验收：湖节点数下降；真多灌 1.56% → 更低；不漏真湖

T3 [—] 河谷剖面（C3）—— ❌ **实测否决，不需要做**（见 §1.17.6）
  T3.1  ✅ 已量：需抬升 7533 列，p50=2.33、p90=13.43、max=47.21 块；99.4% 成带
  T3.2  ❌ 加参数实现：实测空转（仅影响 0.5% 列）⇒ 已撤销（§1.17.5）
  T3.3  ❌ **根本否决**：那些"低于水面却干"的列 **100% 可排水**（§1.17.6）
        ⇒ 现状"干"物理正确 ⇒ "漏灌"概念不成立 ⇒ **C3 无对象**
  T3.4  💡 保留：若将来实机看到"河谷不像河谷"，再考虑【分割剖面】
        （河床/河岸/谷底/渐隐 —— Farseek outletSlopes 语义），属【视觉打磨】非缺陷修复
