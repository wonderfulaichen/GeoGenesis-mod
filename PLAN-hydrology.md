...
第 2 刀  湖域过大（形貌，非性能）      ⏳ 已定位，待定修法
        实测（§1.18）：2127 列"河道外 + 是陆地 + 在河谷带"中，
        1497 列（70.4%）被湖分支接管 ⇒ 这些陆地河谷【完全没被雕】。
        候选：A 湖分支也做谷壁塑形 / B 收窄湖域（★有回归风险）/ C 维持现状。
...
### 1.18.3 修法候选（须先量，勿直接改）

| 方案 | 含义 | 风险 |
|---|---|---|
| **A. 湖分支也做谷壁塑形** | 保留"湖底不雕"，但河谷带仍按河分支塑形 | 需分辨"湖底 vs 谷壁"，逻辑复杂 |
| **B. 收窄湖域** | 只让湖认领真正的水体+岸线附近 | ★ 可能重新引入"硬边"（域边界成为水边）—— 正是 2026-09-19 修复的机制 |
| **C. 维持现状** | 承认湖盆内河谷不塑形 | 若实机看不出，可接受 |

**⚠ 方案 B 有明确的回归风险**：湖的水边由落块侧 `height < spill` 等高线定，
但**只有拿到湖命中的列**才走湖分支；域一收窄，域外列落回河分支 ⇒ 不灌 ⇒
域边界重新成为一条硬边。

**⇒ 必须先量**：收窄到不同半径时，"硬边"是否重现 + "抑制河谷"下降多少。
