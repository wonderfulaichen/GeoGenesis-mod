...
```
T1 [P0] 统一水位场 —— 消掉"海/湖/河"三套基准        ← 症状的共同根
  T1.1  ✅ 已有 WaterField（逐格闭式，跨区不一致 0 vs filledAt 99.9%）
  T1.2  ⏳ 参数定稿：smoothWu=24 / offset=−0.5（实测水格 2.5%、15 个湖）
  T1.3  ⏳ 接线：湖水位改用 W 锚定（溢出口仍局部求，物理正确）
  T1.4  ⏳ 接缝 gasket：邻块水位对齐（闭式 W 使其可行）
  T1.5  ⏳ 验收：runLakeEdgeProbe 接缝不一致 → 0；实机无接缝水墙

T2 [P0] 湖去对象化（C7）—— 从"独立系统"到"节点属性/乘数"
  T2.1  ⏳ 量：湖域 10846 列 vs 实际淹水 2959 列（71.6% 空转）
  T2.2  ⏳ 湖域收窄：inDomain 从 4×gridCell 收紧（或改为连通性驱动）
  T2.3  ⏳ 湖 = 河床半径乘数（RTF 范式）替代独立 LakeNode 分支
  T2.4  ⏳ 验收：真多灌 1.56% → 更低；湖域空转率大幅下降
